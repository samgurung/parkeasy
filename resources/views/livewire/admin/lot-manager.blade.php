<div class="relative min-h-[calc(100dvh_-_4rem)] w-full overflow-hidden bg-[#070312] text-white flex flex-col">
    ...
    <div class="relative z-10 flex w-full max-w-5xl flex-1 flex-col mx-auto px-6 py-6">
        ...
        <section class="rounded-3xl border border-white/15 bg-white/10 px-6 py-6 shadow-2xl backdrop-blur-xl mb-8">
            ...
            <form wire:submit="add" class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div>
                    <label class="text-xs font-bold uppercase tracking-[0.2em] text-white/60" for="name">Parking Lot
                        Name</label>
                ...
                <div>
                    <label class="text-xs font-bold uppercase tracking-[0.2em] text-white/60"
                        for="rateTwoWheeler">2-Wheeler Rate (₹/hr)</label>
                    <input id="rateTwoWheeler" wire:model="rateTwoWheeler" type="number" min="0" step="0.01"
                        placeholder="10"
                        class="mt-1 w-full rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-white placeholder-white/40 outline-none focus:border-sky-400 focus:bg-white/15" />
                    @error('rateTwoWheeler')
                        <p class="mt-1 text-xs text-rose-400">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label class="text-xs font-bold uppercase tracking-[0.2em] text-white/60"
                        for="rateFourWheeler">4-Wheeler Rate (₹/hr)</label>
                    <input id="rateFourWheeler" wire:model="rateFourWheeler" type="number" min="0"
                        step="0.01" placeholder="20"
                        class="mt-1 w-full rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-white placeholder-white/40 outline-none focus:border-sky-400 focus:bg-white/15" />
                    @error('rateFourWheeler')
                        <p class="mt-1 text-xs text-rose-400">{{ $message }}</p>
                    @enderror
                </div>
                <div class="flex items-end sm:col-span-2 lg:col-span-4">
                    <button type="submit"
                        class="w-full md:w-1/3 mx-auto rounded-xl bg-linear-to-br from-sky-400 to-blue-600 px-6 py-3 text-sm font-black uppercase tracking-[0.2em] text-white shadow-lg hover:brightness-110 transition">
                        <i class="fas fa-plus mr-2"></i> Add Parking Lot
                    </button>
                </div>
                <p class="text-xs text-white/40 sm:col-span-2 lg:col-span-4">
                    <i class="fas fa-circle-info mr-1 text-sky-400"></i>
                    The parking lot number is assigned automatically (1, 2, 3, …) and used by the ESP32 to address this
                    parking lot. Exit fees are billed at the rate for the vehicle type chosen on entry.
                </p>
            </form>
        </section>
                ...
                <div class="mb-4 rounded-2xl border border-white/15 bg-white/10 px-6 py-5 shadow-xl backdrop-blur-xl">
                    ...
                    @if ($editingId === $lot->id)
                        ...
                        <form wire:submit="save" class="grid grid-cols-1 gap-4 sm:grid-cols-2 items-end">
                            <div>
                                <label class="text-xs font-bold uppercase tracking-[0.2em] text-white/60">Parking Lot
                                    Name</label>
                            ...
                            <div>
                                <label
                                    class="text-xs font-bold uppercase tracking-[0.2em] text-white/60">Address</label>
                            ...
                            <div>
                                <label class="text-xs font-bold uppercase tracking-[0.2em] text-white/60">2-Wheeler Rate
                                    (₹/hr)
                                </label>
                                <input wire:model="editRateTwoWheeler" type="number" min="0" step="0.01"
                                    class="mt-1 w-full rounded-xl border border-white/20 bg-white/10 px-4 py-2 text-white outline-none focus:border-sky-400 focus:bg-white/15" />
                                @error('editRateTwoWheeler')
                                    <p class="mt-1 text-xs text-rose-400">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                <label class="text-xs font-bold uppercase tracking-[0.2em] text-white/60">4-Wheeler Rate
                                    (₹/hr)</label>
                                <input wire:model="editRateFourWheeler" type="number" min="0" step="0.01"
                                    class="mt-1 w-full rounded-xl border border-white/20 bg-white/10 px-4 py-2 text-white outline-none focus:border-sky-400 focus:bg-white/15" />
                                @error('editRateFourWheeler')
                                    <p class="mt-1 text-xs text-rose-400">{{ $message }}</p>
                                @enderror
                            </div>
                    ...
                    @else
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div class="flex items-center gap-4">
                                <span
                                    class="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-teal-400 to-emerald-600 text-xl font-black text-white shadow">
                                    {{ $lot->lot_number }}
                                </span>
                                <div>
                                    ...
                                    <div class="mt-1 text-xs text-white/50">
                                        <i
                                            class="fas fa-motorcycle mr-1 text-sky-300"></i>₹{{ number_format($lot->rate_two_wheeler, 2) }}/hr
                                        &bull; <i
                                            class="fas fa-car mr-1 text-teal-300"></i>₹{{ number_format($lot->rate_four_wheeler, 2) }}/hr
                                    </div>
                                </div>
                            </div>
    ...
    @if ($deletingId)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md">
            <div
                class="result-pop relative w-full max-w-sm rounded-[2rem] border border-rose-400/30 bg-white/15 p-10 text-center shadow-2xl backdrop-blur-2xl">
                ...
                <p class="text-sm text-white/60 mb-6">This will permanently delete the parking lot and all its floors
                    and slot data.</p>
